...
from gaussian_renderer import render_nerf
import torchvision
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, render_normals):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        render_pkg = render_nerf(view, gaussians, pipeline, background, iteration=iteration) 
        rendering = render_pkg["pbr_rgb"] * render_pkg["rend_alpha"] + (1-render_pkg["rend_alpha"]) * background[:, None, None]
        ...
        if render_normals:
            normals = render_pkg["rend_normal"] * render_pkg["rend_alpha"]
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, render_normals : bool):
    with torch.no_grad():
        ...
        if not skip_train:
             render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, render_set)
